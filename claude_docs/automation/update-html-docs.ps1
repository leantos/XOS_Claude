...
$claudeDocsPath = Split-Path -Parent $scriptDir
$htmlOutputPath = Join-Path $claudeDocsPath "html-docs"
$templatePath = Join-Path $claudeDocsPath "html-template.html"
$backupPath = Join-Path $claudeDocsPath ".backup-html"
...
function Update-CrossReferences {
    ...
    # Handle @claude_docs\ references (Windows paths) - convert to relative paths
    $content = $content.Replace('@claude_docs\', '../')
    $content = $content.Replace('@claude_docs/', '../')
    
    # Convert .md extensions to .html in links and references
    $content = $content.Replace('.md"', '.html"')
    $content = $content.Replace('.md#', '.html#')
    $content = $content.Replace('.md)', '.html)')
    
    # Handle href attributes specifically
...
if ($orphanedFiles.Count -gt 0) {
    Write-Host "Found $($orphanedFiles.Count) HTML files without corresponding MD files:" -ForegroundColor Yellow
