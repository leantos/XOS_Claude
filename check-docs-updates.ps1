...
<#
.SYNOPSIS
    Check for claude_docs updates and manage documentation synchronization
...
$script:ClaudeDocsPath = Join-Path $script:ProjectPath "claude_docs"
...
function Detect-SetupType {
    ...
    if (Test-Path "$script:ProjectPath\update-html-docs.ps1") {
        return "main-repo"
    }
...
function Show-UpdateChangelog {
    ...
    try {
        ...
        if ($deletedFiles) {
            Write-Host "🗑️  Removed files:" -ForegroundColor Red
        ...
        switch ($SetupType) {
            "main-repo" {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    if (Test-Path "update-html-docs.ps1") {
            ...
            "submodule" {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    if ($LASTEXITCODE -ne 0) {
                        ...
                        git add claude_docs
                        git commit -m "📚 Update claude_docs submodule to latest

🤖 Auto-updated by check-docs-updates.ps1 v$script:Version"
...
if ($Scheduled) {
    exit ($script:UpdatesAvailable ? 1 : 0)
}